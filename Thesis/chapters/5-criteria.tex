...
\mathit{A} := {Architectures}& a in \mathit{A}


\mathit{P} := {Attack Paths_{a}}& p in \mathit{P} \\\\
...
\begin{cases}
1,&\text{base case}\\
cgw_{a} - 0.2,&\text{if not in a}\\
cgw_{a} - 0.3,&\text{if external interface in a}\\
cgw_{a} - 0.1,&\text{if target in in a}\\
\end{cases}\\\\

hops_{a} := \sum_{p} hops_{p}\\\\

interfaces := \text{total amount of entry points in } a\\\\

separation := \text{average amount of ECUs per bus in } a\\\\

rating_{a} := \frac{100 * feasibility_{a} * cgw_{a}}{hops_{a}*w_{1} + isolation_{a}^{w_{2}} + interfaces_{a}*w_{3}}